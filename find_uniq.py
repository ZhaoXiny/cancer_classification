...
test_v = pd.read_csv('./data/stage2_test_variants.csv')
#train_t = pd.read_csv('../input/training_text',sep='\|\|',skiprows=1,engine='python',names=["ID","Text"])
test_t = pd.read_csv('./data/stage2_test_text.csv',sep='\|\|',skiprows=1,engine='python',names=["ID","Text"])

train = train_v = pd.read_csv('./data/stage_2_train.csv')
...
uniqsTextMatrTest=[]
for file in texts_for_test:
    uniqsTextMatrTest.append(termsComps(file))

uniqTextTest = pd.DataFrame(uniqsTextMatrTest, columns=['class'+str(c+1) for c in range(9)])

uniqTextTest.to_csv('uniqtest.csv',index=False)
